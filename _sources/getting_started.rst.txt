...
ell's autocommit feature uses Kimi K3 (``moonshotai/Kimi-K3``, served from a Modal Endpoint; set ``MODAL_PROXY_TOKEN_ID`` / ``MODAL_PROXY_TOKEN_SECRET`` to a Modal proxy token pair) to generate these commit messages automatically, providing a clear history of how your LMPs evolve. Pass ``autocommit_model=`` to ``ell.init`` to use a different model.
